...
ax=fig.add_subplot(gs[0,:]);ax.set_facecolor('#13233b');ax.axis('off');ax.text(.02,.65,'Reaction Mechanism Digital Twin',color='white',fontsize=24,weight='bold');ax.text(.02,.25,'Open-source, vendor-neutral pathway validation | MIT | vicena.ai',color='#b9d7ff',fontsize=11)
...
b=fig.add_subplot(gs[2,2:]);b.axis('off');b.text(.02,.9,'Define  >  Optimize  >  Find TS  >  Validate Path  >  Compare',fontsize=12,weight='bold');b.text(.02,.6,'Verified release metrics: 12 local tests, 0 paid jobs,\n1 synthetic imaginary mode, 2 synthetic IRC directions.\nThese values test software, not chemical validity.',fontsize=10);b.text(.02,.15,'https://github.com/vicena-labs/reaction-mechanism-digital-twin',fontsize=9)
fig.tight_layout();fig.savefig(R/'assets/reaction-mechanism-digital-twin-onepager.png',dpi=160);fig.savefig(R/'Reaction_Mechanism_Digital_Twin_OnePager.pdf');plt.close(fig)
