...
h.text(.137,.66,"REACTION MECHANISM DIGITAL TWIN",transform=h.transAxes,color="white",fontsize=25,weight="bold")
h.text(.137,.31,"A vendor-neutral R&D baseline for pathway evidence, transition states and kinetics, built with Vicena",transform=h.transAxes,color="#DCE2EA",fontsize=12.5)
...
m.text(0,.72,"Turn structures and saved quantum outputs into an inspectable mechanism workflow.",fontsize=17.5,weight="bold",color=C["dark"],transform=m.transAxes)
m.text(0,.28,"Validate metadata, prepare endpoints, interrogate transition states, preserve failures, and compare calculated evidence with experiment.",fontsize=11,color=C["text"],transform=m.transAxes)

# Four capability cards
cards=[("INPUT READY","Mapped structures • charge • spin","Units • conditions • provenance"),("ENDPOINT MODELING","Local geometries • Rowan conformers","Saved UUIDs • raw outputs"),("TS ACCEPTANCE","Imaginary mode • geometry • mapping","IRC or equivalent path evidence"),("AGENT ADAPTABLE","Schemas • playbooks • tests","New reactions without rewrites")]
...
# Workflow strip
wf=fig.add_subplot(g[76:91,3:97]);wf.axis("off");wf.text(.02,.88,"ONE PLATFORM, AN AUDITABLE MECHANISM WORKFLOW",fontsize=12.5,weight="bold",color=C["dark"],transform=wf.transAxes)
steps=[("DEFINE","structures + metadata"),("PREPARE","endpoint conformers"),("SEARCH","transition-state guess"),("VALIDATE","frequency + path"),("COMPARE","experiment + uncertainty")]
for i,(a,b) in enumerate(steps):
    xx=.01+i*.2
    wf.add_patch(FancyBboxPatch((xx,.05),.165,.58,boxstyle="round,pad=.02,rounding_size=.04",facecolor=C["light"],edgecolor=C["line"],lw=1.1,transform=wf.transAxes))
    wf.text(xx+.0825,.39,a,ha="center",fontsize=9.5,weight="bold",color=C["dark"],transform=wf.transAxes);wf.text(xx+.0825,.19,b,ha="center",fontsize=8,color=C["text"],transform=wf.transAxes)
    if i<4: wf.add_patch(FancyArrowPatch((xx+.17,.34),(xx+.195,.34),arrowstyle="-|>",mutation_scale=13,color=C["gold"],lw=2,transform=wf.transAxes))

# Footer
f=fig.add_subplot(g[93:100,:]);f.set_facecolor(C["light"]);f.set_xticks([]);f.set_yticks([])
for s in f.spines.values():s.set_visible(False)
f.text(.04,.48,"github.com/vicena-labs/reaction-mechanism-digital-twin",fontsize=10,weight="bold",color=C["dark"],transform=f.transAxes)
...
fig.savefig(ROOT/"Reaction_Mechanism_Digital_Twin_OnePager.pdf",dpi=180,facecolor="white")
fig.savefig(A/"reaction-mechanism-digital-twin-onepager.png",dpi=180,facecolor="white")
plt.close(fig)
